...
example, Barba and Morin \cite{bmXX} found that a \naive\ implementation
...
This problem is hinted at in the experimental results of Brodal \etal\
\cite[Section~XX]{X} and examined in detail by the first author \cite{X},
...
A branchy implementation of search in an Eytzinger array is nearly
...
I eytzinger_array<T,I,aligned>::branchy_search(T x) const {
	I i = 0, j = n;
	while (i < n) {
		if (x < a[i]) {
			j = i;
...
A branch-free implementation of search in an Eytzinger array is also quite
simple. Unfortunately, unlike branch-free binary search, the branch-free
Eytzinger search is unable to avoid variations in the number of iterations
of the while loop, since this depends on the depth (in the virtual tree)
of the leaf that is reached when searching for \mintinline{c++}{x}.
When $n=2^h-1$ for some integer $k$, then all leaves have the same depth
but, in general, there will be leaves of depths $h$ and depth $h-1$,
where $h=\lceil\log (n+1)\rceil-1$ is the height of the virtual tree.

\begin{listing}
\begin{minted}[linenos]{c++}
template<typename T, typename I, bool aligned>
I eytzinger_array<T,I,aligned>::branchfree_search(T x) const {
    I i = 0, j = n;
    while (i < n) {
        j = (x <= a[i]) ? i : j;
        i = (x <= a[i]) ? (2*i + 1) : (2*i + 2);
...
Eytzinger search has smaller constants is the inner binary search of
...
behaviour.  This is because 17-trees essentially synchronously
...
Finally, we remark on the ``bumpiness'' of the Eytzinger array search
times in \figref{btree-ii}.  This bumpiness is caused by branch
mispredictions during the execution of the while loop.  The search
...
the final iteration of the while loop.
...
\begin{lesson}
  ...
  B-trees.  It does this by exploiting parallelism in the memory subsystem
  and between the processor and memory subsystem.
\end{lesson}
...
which corresponds, roughly, to the first $\log C$ iterations of the search
...
However, to search in a van Emde Boas layout one needs code that
can determine the indices of the left and right child of each node.
After experimenting with several different methods of doing this, we found
it most efficient to store three precomputed lookup tables whose size is
equal to the height of the van Emde Boas tree. In addition to these lookup
tables, one must also store the path, \mintinline{c++}{rtl}, from the
...
layout.  The most obvious (branchy) code implementing this algorithm is
shown in \lstref{veb}
