...
\item We measured L3 cache accesses using
   ...
   $2\times 10^6$ searches.  The branch-free code performs only $8.4\times
...
the while loop.
For example, measuring with \mintinline{sh}{perf} when $n=2^{11}$ or
$n=2^{12}$, we find that the branch-free Eytzinger search has a branch-misprediction
rate of $0.04\%$ when performing $2\times 10^{7}$ searches.  When
$n=(2^{11}+2^{12})/2$, the branch misprediction rate is $1.45\%$.
