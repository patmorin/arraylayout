...
For a single example of how useful sorting/binary
search is, consider the chromium browser code-base
...
include a mask as described in \secref{other-machines}.}
...
into the CPU. From the time the CPU issues the read request to the time
...
Thus, on average, we expect binary search to incur roughly $\log n -
\log(C/B) - \log B + 1 = \log n - \log C + 1$ cache misses. The first
$\log(C/B)$ comparisons are to cached values and the last $\log B$
comparisons are all to values in the same cache line.
...
search). \Figref{mixed-ii} compares branch-free Eytzinger with prefetching
...
On our machine, $WL=3.124$ and, with 4-byte data, $B=16$, so $\log B=4$,
and the prefetching Eytzinger search has four outstanding cache line
requests at any given time.  Since $4>3.124$, the Eytzinger search
algorithm is already saturating the memory bandwidth.  Taking $t=1$
or $t=2$ attempts to even further saturate the bandwith by having $8$,
respectively 16, simultaneous cache line requests.
...
This model also suggests a \emph{simulation}, in the sense that, for any $k\le
W$ one can load $k$ cache lines in $L+r(k)$ time units, where $r(k)$
is the time it takes to issue $k$ prefetch requests.  By doing this, one
can simulate a model with cache line width $B'=kB$ and latency $L=L+r(k)$.
...
$k$ cache lines used by that node.  \Figref{bktree} shows the results of
...
It lead us to deeper prefetching in the Eytzinger search algorithm and the $(Bk+1)$-tree, which behave more or less as the model predicts.
