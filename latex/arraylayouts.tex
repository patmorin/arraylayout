...
$k$ cache lines used by that node.  \Figref{bqtree} shows the results of
implementing this for various values of $k$.  We tested values $k\in\{1,\ldots,16\}$, but \figref{bqtree} only shows the fastest values.

\begin{figure}
   \centering{\includegraphics{figs/bqtrees}}
...
some validation of the theoretical model proposed above.  For some
choices of $k$ and some values of $n$, this yields a layout that is faster
than the Eytzinger layout. Unfortunately, it requires considerably more
...
With the exception of \figref{atom-i}, the results presented here are
