...
value \mintinline{c++}{hi} stores the return value. In Eytzinger search,
we must decode the return value from the value of \mintinline{c++}{i}
at the end of the \mintinline{c++}{while} loops.  Since we haven't seen
...
In our array, the $2^d$ nodes of the virtual tree at depth $d$
...
compilers \cite{wikipedia:find-first-set}.
...
that are highly predictable.  Our results show that this kind of
...
a factor of $d$.  There is \ldots

Although the Eytzinger memory access pattern is predictable,
it is still well beyond the capabilities of current hardware prefetchers.
It would be interesting to know if other algorithmic problems could be 
...
subsystem.  Indeed, speculative execution is a form of prefetching that
can speed up memory access patterns that are far too complicated for
current prefetching techniques.  In extreme cases (like the Eytzinger
layout) the resulting prefetches are perfectly correct even four branches
ahead of the current execution point.

In hardware design, the general idea of using speculative execution as a
prefetching strategy is called \emph{runahead} \cite{blah}.  Runahead and
runahead and other forms of speculative prefetching were part of the
hardware scout technology proposed for the Sun Rock processor project,
which was cancelled before the processor made it into production.

Our results show that, in a processor that has speculative execution,
one can obtain many of the benefits of runahead even on processors
that do not specifically implement it. As a more general guideline, our
results show that one has to be very careful with micro-optimizations.
Optimizations that offer great speedups on small inputs can create great
slowdowns on large inputs.
