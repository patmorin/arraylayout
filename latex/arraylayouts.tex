...
Consider the chromium browser code-base which calls these
...
This is the code-base on which Google Chrome, which now boasts
over a billion users \cite{protalinksi:google}, is based. It is
not an exaggeration to say that faster
implementations of binary search could improve the daily user
experience of hundreds of millions of people.
...
\begin{itemize}
\item Show results on different data sizes: 8- and 16-byte data.
\item Show results when the number of threads is increased.
\item Discuss results on other machines.
\end{itemize}
