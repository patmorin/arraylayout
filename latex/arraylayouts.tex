...
A branchy implementation of search in an Eytzinger array is nearly as
simple as that of binary search, with only one slight complication. When
branchy binary search completes, the value \mintinline{c++}{hi} stores the
return value. In Eytzinger search, we must decode the return value from
the value of \mintinline{c++}{i} at the end of the \mintinline{c++}{while}
loops.  Since we haven't seen this before, we explain it here.

The array element stored at position \mintinline{c++}{i} corresponds
to a node in the virtual tree that has some depth $d$. Since the
number of nodes at depth $d$ is $2^d$, notice that $i+1$ is the range
$\{2^d,\ldots,2^{d+1}-1\}$.  Therefore, the binary representation\ldots

; the only additional complication is
...
\Lstref{eytzinger-iii} shows another optimized implementation that avoids
explicitly keeping track of the value \mintinline{c++}{j}.  It uses the
fact that, after the \mintinline{c++}{while} loop terminates, the value
of \mintinline{c++}{i + 1} is a one bit followed by the search path
encoded as a sequence of zeros and ones, denoting left and right turns,
respectively. The code then extracts the last place the search path turned
left (a zero bit in i+1), chops off all trailing right turns (one bits)
and then turns the result back into an array location by subtracting one.
