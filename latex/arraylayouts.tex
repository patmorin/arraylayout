...
quite simple. Unfortunately, the branch-free version is unable to avoid
variations in the number of iterations of the while loop, since this depends on the depth of the leaf that is reached when searching for \mintinline{c++}{x}.
...
branch-free binary search. As expected, there is an increasing in slope
at $n=2^{21}$ since the cache on our test machine can store $C=2^{21}$
4-byte values.
...
\begin{figure}
   \centering{\includegraphics[width=.8\textwidth]{figs/eytzinger-i}}
...
distance $\ell$ of $u$ are consecutive and occupy array indices $2^\ell
...
15/16) guess an execution path that loads the correct cache line.
...
\begin{enumerate}
  ...
  \item We can align the array so that \mintinline{c++}{a[0]} is the second element of a cache line.  In this way, all 16 great-great grandchildren of a node will always be stored in a single cache line.
\end{enumerate}

The results of implementing both of these optimizations are shown in \figref{eytzinger-ii}.
