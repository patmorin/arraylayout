...
\received[revised]{August 2016}
\received[accepted]{January 2017}
...
\keywords{Binary search, data layouts, microprocessor architecture, 
   caching, pipelining}
...
\begin{enumerate}
  ...
  \begin{figure}
    \begin{center}\includegraphics[scale=0.9]{eytzinger}\end{center}
...
%  at $A[i]$ are stored at $A[2i+1]$ and $A[2i+2]$, respectively.
  ...
  \begin{figure}
    \begin{center}\includegraphics[scale=0.9]{btree}\end{center}
...
%  beginning at position $i(b+1)+(j+1)b$.
  ...
  \begin{figure}
    \begin{center}\includegraphics[scale=0.9]{veb}\end{center}
...
search (see Point~2 in the next Section and \figref{eytzinger-iii}).
...
for example, our \figref{eytzinger-iii} at the value $n=2^8$.
...
The compiler used was \mintinline{sh}{gcc} version 4.8.4 with the command
line arguments \\
\centerline{  \mintinline{sh}{-std=c++11 -Wall -O4 -march=native} \enspace . }
...
when searching for a value \mintinline{c++}{x} that is not present, unless
$n$ has the form $2^k-1$, the exact number of iterations is different for
different values of \mintinline{c++}{x}.  This implies that the branch
...
\Figref{sorted-iii} compares the performance of the branchy and
...
using the \mintinline{sh}{perf} performance monitoring tool explains
...
\item We measured L3 cache accesses using
   \mintinline{sh}{perf}.\footnote{For this, we used the command
   \mintinline{sh}{perf stat -e r4f2e} to obtain the number of lowest
...
Something else evident in \figref{eytzinger-ii} is a ``bumpiness'' of the
...
\mintinline{sh}{perf} when $n=2^{11}$ or $n=2^{12}$, we find that the
...
is also optimal in practice (see \figref{btrees}).

\begin{figure}
   \begin{center}
      \includegraphics{figs/btrees}
   \end{center} 
